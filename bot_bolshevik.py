...
async def processclasschoice(callbackquery: types.CallbackQuery):
    ...
    cursor.execute("INSERT INTO killers (userid, firstname, class) VALUES (?, ?, ?) ON CONFLICT(userid) DO UPDATE SET firstname = ?, class = ?", (userid, firstname, newclass, firstname, newclass))
    conn.commit()
...
async def killcmd(message: types.Message):
    ...
    if random.randint(1, 100) <= injury_chance:
        if shields > 0:
            cursor.execute("UPDATE killers SET shields = shields - 1 WHERE userid = ?", (userid,))
            conn.commit()
    ...
    cursor.execute("""
        ...
        ON CONFLICT(userid) DO UPDATE SET 
            firstname = ?, kills = kills + ?, coins = coins + ?, lastkill = ?, boost_atk = 0, boost_coins = 0
